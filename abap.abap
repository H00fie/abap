...
**************************************
*BACK-UP ZWIEK, GDY¯ PRÓBUJÊ KOPIUJ-WKLEJ ¯EBY "OBEJŒÆ" TÊ POGMATWAN¥ LOGIKÊ
...
*
**Siêgniêcie po numery partii materia³ów. Dane s¹ w œrodku. Numer materia³u i numer partii.
...
*t_main-ran2 = 20.
*t_main-ran3 = 30.
**Wartoœci udaje siê przypisaæ, to dzia³a.
*APPEND t_main.
**t_main tu wy¿ej to de facto obszar roboczy i muszê dodaæ APPENDa, ¿eby dodaæ to do tabeli.
*WRITE: / t_main-matnr, t_main-ran1, t_main-ran2, t_main-ran3.
*ULINE.
*WRITE: 'Below should be the logic!'.
*ULINE.
****************************************************************************************************************************************************
**TO JEST DZIA£AJ¥CA LOGIKA ZAPISU I WYŒWIETLANIA
****************************************************************************************************************************************************
**Testowanie idei na wyœwietlanie tego samego rekordu z tabeli tyle razy ile jest numerów partii z innym tym¿e numerem za ka¿dym razem.
...
*ULINE.
*
**Gówno nie dzia³a!!!!!!!!!!!!!! ... W sumie ju¿ dzia³a, chyba dziêki APPEND przy t_main.
...
**  gt_fieldcatalog-seltext_m  = 'nr partii materia³ów'.
**  gt_fieldcatalog-outputlen  = 10.
**  APPEND gt_fieldcatalog TO gt_fieldcatalog.
**  CLEAR  gt_fieldcatalog.
**
...
*    WHERE matnr IN ( SELECT matnr FROM mseg WHERE insmk = 3 ). "DODAJ JAKIEŒ MATNR'y
*
...
*ENDLOOP.
*
**No dzia³a, ale w sumie nie wiem co mi to daje. :< Muszê znaleŸæ jakiœ MATNR maj¹cy ró¿ne INSMKi.
...
*p_matnr = 'W0430'.
*
**Zwraca wszystko oprócz CHARGów
...
*ABSOLUTNIE FENOMENALNE KOD SKOPIOWANY Z ZWM_WIEKOWANIE_PARTIA, KTÓRY CHYBA S£U¯Y£ MI NA POCZ¥TKU, GDY PODJ¥£EM PRÓBÊ ZROBIENIA GO OD NOWA
...
****START -> OSTATECZNY KOD ZWM_ZWIEKOWANIE_PARTIA, KTÓRY USUWA PUSTE CHARGi ORAZ DUPLIKATY****
...
*    ENDLOOP.
*ENDLOOP.
****END -> OSTATECZNY KOD ZWM_ZWIEKOWANIE_PARTIA, KTÓRY USUWA PUSTE CHARGi ORAZ DUPLIKATY****
